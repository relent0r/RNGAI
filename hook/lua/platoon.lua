                                ...
                                if VDist2Sq(scoutPos[1], scoutPos[3], targetData.Position[1], targetData.Position[3]) > 14400 then
                                    ...
                                    if RNGGETN(enemyUnitCheck) > 0 then
                                        for _, v in enemyUnitCheck do
                                            if not v.Dead then
                                                self:Stop()
                                                self:MoveToLocation(RUtils.AvoidLocation(v:GetPosition(), scoutPos, intelRange - 10), false)
                                                break
                                            end
                                        end
                                    end
                        ...
                        for i=1, pathNodesCount do
                            ...
                            while PlatoonExists(aiBrain, self) do
                                ...
                                if enemyUnitCount > 0 then
                                    ...
                                    target = RUtils.AIFindBrainTargetInCloseRangeRNG(aiBrain, self, platoonPos, 'Attack', enemyRadius, categories.MOBILE * (categories.NAVAL + categories.AMPHIBIOUS) - categories.AIR - categories.SCOUT - categories.WALL, atkPri, false)
                                    local attackSquad = self:GetSquadUnits('Attack')
                                    IssueClearCommands(attackSquad)
                                    while PlatoonExists(aiBrain, self) do
                                        if target and not target.Dead then
