...
GameMap = Class({
    InitMap = function(self)
        LOG('RNGAI FlowAI Framework : CreateMapMarkers() started!')
        local START = GetSystemTimeSecondsOnlyForProfileUse()
        self:CreateMapMarkers()
        self:InitZones()
        local END = GetSystemTimeSecondsOnlyForProfileUse()
        LOG(string.format('RNGAI FlowAI Framework : CreateMapMarkers() finished, runtime: %.2f seconds.', END - START ))
...
-- Run in siminit as part of the BeginSession hook
-- GameMap class has the drawStuffz bool for visuals
...
function SetMarkerInformation(aiBrain)
    ...
    LOG('MassPointCount '..massPointCount)
    LOG('Set brain intel for mass marker count')
    aiBrain.BrainIntel.MassMarker = massPointCount
    LOG('Brain intel set, move to next renderthreadtracker')
    LOG('renderthreadtracker for masspoints')
    while aiBrain.renderthreadtracker do
        coroutine.yield(2)
    end
    LOG('renderthreadtracker for masspoints complete')
    --LOG('RNGAreas:')
