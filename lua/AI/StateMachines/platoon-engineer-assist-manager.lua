...
AIPlatoonEngineerAssistManagerBehavior = Class(AIPlatoonRNG) {
    ...
    UpdateRoster = State {
        ...
        Main = function(self)
            ...
            aiBrain.EngineerAssistCurrentBPAllocated = { Energy = 0, None = 0 }
            ...
            for _, eng in platUnits do
                if eng and (not eng.Dead) and (not eng:BeenDestroyed()) then
                    ...
                    -- Allocation tracking (paste your focus-category allocation logic here)
                    -- if aiBrain.EngineerAssistManagerFocusCategory and eng.UnitBeingAssist ... then ...
                    -- update aiBrain.EngineerAssistCurrentBPAllocated[matchingRuleKey]
            ...
            LOG('TotalTechBuildRate[1] '..tostring(self.TotalTechBuildRate[2]))
            LOG('TotalTechBuildRate[1] '..tostring(self.TotalTechBuildRate[3]))
    ...
    CollectAvailable = State {
        ...
        Main = function(self)
            local available = {}
            for _, eng in self:GetPlatoonUnits() do
                if eng and not eng.Dead and not eng.UnitBeingAssist then
                    RNGINSERT(available, eng)
                end
            end
            LOG('Number of available engineers '..tostring(RNGGETN(available)))
        ...
        Main = function(self)
            ...
            for _, assistData in aiBrain.EngineerAssistManagerPriorityTable do
                ...
                if assistData.type == 'Upgrade' then
                    local assistDesc = aiBrain:GetUnitsAroundPoint(assistData.cat, managerPosition, engineerRadius, 'Ally')
                    if assistDesc then
                        local low = false
