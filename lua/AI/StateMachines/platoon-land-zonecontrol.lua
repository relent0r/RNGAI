...
AIPlatoonBehavior = Class(AIPlatoonRNG) {
    ...
    Start = State {
        ...
        Main = function(self)
            ...
            self:LogDebug(string.format('Zone type is '..tostring(self.ZoneType)))
            ...
            if type(self.PlatoonData.MaxPathDistance) == 'string' then
                ...
                self.MaxPathDistance = self.PlatoonData.MaxPathDistance or 250
            end
        ...
        Main = function(self)
            ...
            if not targetZone then
                if self.PlatoonData.EarlyRaid then
                    ...
                    local transportsRequired = 3
                    if self.Raid then
                        transportsRequired = 1
                    end
                    if self.MovementLayer == 'Land' and aiBrain:GetCurrentUnits(categories.TRANSPORTFOCUS) < transportsRequired then
                        currentLabel = true
                    end
        ...
        Main = function(self)
            ...
            if target and not target.Dead then
                ...
                if targetDistance < targetRange * targetRange then
                    self:LogDebug(string.format('We are within enemy weapon range'))
                    if zonePos then
                        for _, v in platUnits do
                            StateUtils.IssueNavigationMove(v, zonePos)
                        end
                        self:LogDebug(string.format('We are moving straight to the closest zone'))
                        coroutine.yield(30)
                    else
                    ...
                    self:LogDebug(string.format('We are outside of enemy weapon range'))
                    local targetCats = target.Blueprint.CategoriesHash
                    ...
                    if attackStructure then
                        ...
                        if zoneRetreat then
                            self:LogDebug(string.format('We are retreating to the closest zone'))
                            for _, v in platUnits do
                                if not v.Dead then
                                    StateUtils.IssueNavigationMove(v, zonePos)
                                end
                            ...
                            self:LogDebug(string.format('We are retreating home'))
                            for _, v in platUnits do
                                if not v.Dead then
            ...
            self.dest = self.BuilderData.Position
            self:LogDebug(string.format('We are going to navigate to somewhere '..tostring(repr(self.BuilderData.Position))))
            self:ChangeState(self.Navigating)
            return
        end,
    },
                ...
                if self.MovementLayer == 'Land' and self:CalculatePlatoonThreat('Surface', categories.AMPHIBIOUS + categories.HOVER) > 0 then
                    ...
                    if NavUtils.CanPathTo('Amphibious', self.Pos, builderData.Position) then
                        ...
                        if table.getn(amphibiousSurfaceUnits) > 0 then
                            ...
                            import("/mods/rngai/lua/ai/statemachines/platoon-land-zonecontrol.lua").AssignToUnitsMachine({ PlatoonData = self.PlatoonData }, plat, plat:GetPlatoonUnits())
                            coroutine.yield(2)
                        end
                    end
                end
            ...
            if not self.path and self.BuilderData.Position and self.BuilderData.CutOff then
                ...
                if not path then
                    ...
                    if reason ~= "TooMuchThreat" then
                        ...
                        return
                    elseif (reason == "TooMuchThreat" and table.getn(threats) > 0) and NavUtils.CanPathTo(self.MovementLayer, self.Pos,self.BuilderData.Position) then
