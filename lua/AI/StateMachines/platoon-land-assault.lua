    ...
    DecideWhatToDo = State {
        ...
        Main = function(self)
            ...
            if target and not IsDestroyed(target) then
                ...
                if targetDistance < self.EnemyRadiusSq and NavUtils.CanPathTo(self.MovementLayer, self.Pos, targetPos) then
                    --self:LogDebug('target close in DecideWhatToDo, CombatLoop')
                    local threat=RUtils.GrabPosDangerRNG(aiBrain,targetPos,self.EnemyRadius * 0.7, self.EnemyRadius, true, false, true, true)
                    local platoonTotalSurfaceThreat = self.CurrentPlatoonThreatDirectFireAntiSurface + self.CurrentPlatoonThreatIndirectFireAntiSurface
                    if (threat.enemyStructure > 0 or threat.enemySurface > 0) and threat.enemyrange > self['rngdata'].MaxPlatoonWeaponRange and (threat.allySurface + platoonTotalSurfaceThreat) < (threat.enemyStructure + threat.enemySurface * 1.3) then
                        --LOG('Enemy Position outranges us')
                        ...
                        coroutine.yield(5)
                        self:ChangeState(self.DefenseLoiter)
                        return
                    end
                    if rangedAttack then
                ...
                if self.MovementLayer == 'Land' and self:CalculatePlatoonThreat('Surface', categories.AMPHIBIOUS + categories.HOVER) > 0 then
                    ...
                    if NavUtils.CanPathTo('Amphibious', self.Pos, builderData.Position) then
                        ...
                        if table.getn(amphibiousSurfaceUnits) > 0 then
                            ...
                            import("/mods/rngai/lua/ai/statemachines/platoon-land-assault.lua").AssignToUnitsMachine({ PlatoonData = self.PlatoonData }, plat, plat:GetPlatoonUnits())
                            coroutine.yield(2)
                        end
                    end
                end
