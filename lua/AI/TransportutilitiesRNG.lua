...
function ReturnTransportsToPool( aiBrain, units, move )
	...
	if TransportDialog then
		LOG("*AI DEBUG "..aiBrain.Nickname.." transport returning transports to pool")
		for _, v in units do
			if not v.Dead then
			    LOG('Unit returned '..v.EntityId)
			end
		end
	end
...
function SendPlatoonWithTransports(aiBrain, platoon, destination, attempts, bSkipLastMove, platoonpath )
	...
	if MovementLayer == 'Land' or MovementLayer == 'Amphibious' then
		...
		if (not bUsedTransports) then
			if transportplatoon then
				if TransportDialog then	
                    LOG("*AI DEBUG "..aiBrain.Nickname.." SendPlatWTrans "..repr(platoon.BuilderName).." "..repr(platoon.BuilderInstance).." not bUsedTransports return transports to platoon" )
                end
		...
		if (not platoon) or (not PlatoonExists(aiBrain, platoon)) or (not bUsedTransports) then
			...
			if PlatoonExists(aiBrain,transportplatoon) then
				if TransportDialog then
                    LOG("*AI DEBUG "..aiBrain.Nickname.." "..repr(platoon.BuilderName).." "..transportplatoon.BuilderName.." didnt use transports return them to pool")
                end
				ForkTo( ReturnTransportsToPool, aiBrain, GetPlatoonUnits(transportplatoon), true)
			end
			return false
...
function UseTransports( aiBrain, transports, location, UnitPlatoon, IsEngineer )
	...
	if counter > 0 then
		...
		if currLeftovers[1] then
			...
			if TransportDialog then
				LOG("*AI DEBUG TRANSPORT "..repr(unit.PlatoonHandle.BuilderName).." Transport "..repr(currLeftovers[1].EntityId).." RTB - Excess in SortingOnTransport" )
			end
			returnpool:SetAIPlan('ReturnToBaseAI',aiBrain)
		end
	end
	...
	if loading then
		...
		if not PlatoonExists(aiBrain, transports) then
			if TransportDialog then
				LOG("*AI DEBUG transports platoon no longer exists going into load watch")
			end
		else
			if TransportDialog then
				LOG("*AI DEBUG transports platoon exists going into load watch")
			end
		end
		...
		while loadwatch do
			...
			if PlatoonExists( aiBrain, transports) then
				...
				if TransportDialog then
					LOG("*AI DEBUG transports platoon no longer exists within loadwatch")
				end
		    end
		end
	end
    ...
    if TransportDialog then
		...
		LOG('EntityID '..repr(UnitPlatoon:GetPlatoonUnits()[1].EntityId))
	end

	if not PlatoonExists(aiBrain, transports) then
		if TransportDialog then
			LOG("*AI DEBUG transports platoon not longer exists, return false")
		end
        UnitPlatoon.UsingTransport = false
		return false
	end
		...
		for k,v in GetPlatoonUnits(UnitPlatoon) do
			if v and (not v.Dead) then
				if not IsUnitState( v, 'Attached') then
					...
					if TransportDialog then
						LOG("*AI DEBUG "..aiBrain.Nickname.." "..UnitPlatoon.BuilderName.." "..transports.BuilderName.." failed to load unit")
						LOG('*AI DEBUG adding unit to returnpool '..v.EntityId)
					end
					IssueClearCommands(v)
	...
	if PlatoonExists( aiBrain, transports ) then
		for k,v in GetPlatoonUnits(transports) do
			if v and (not v.Dead) and TableGetn(v:GetCargo()) == 0 then
				if TransportDialog then
					LOG("*AI DEBUG "..aiBrain.Nickname.." "..UnitPlatoon.BuilderName.." "..transports.BuilderName.." failed to load unit")
					LOG('*AI DEBUG adding unit to ReturnTransportsToPool '..v.EntityId)
				end
	...
	if not PlatoonExists(aiBrain,UnitPlatoon) then
		if TransportDialog then
			LOG("*AI DEBUG UnitPlatoon platoon not longer exists, return false")
		end
        return false
    end
		...
		if (not transport.Dead) and transport.Loading and ( not IsUnitState(transport,'Moving') or IsUnitState(transport,'TransportLoading') ) then
			...
			if unitsdead or (not loading) or reloads > 20 then
				if unitsdead then
                    transport.Loading = nil
					if TransportDialog then
						LOG("*AI DEBUG "..aiBrain.Nickname.." "..UnitPlatoon.BuilderName.." "..transport.PlatoonHandle.BuilderName.." Transport "..transport.EntityId.." not loading or reloads > 20")
					end
					ForkTo ( ReturnTransportsToPool, aiBrain, {transport}, true )
                    return
				end
...
function WatchTransportTravel( transport, destination, aiBrain, UnitPlatoon )
	...
	while (not transport.Dead) and (not unitsdead) and transport.Travelling do
			...
			if unitsdead and not EntityCategoryContains(categories.uea0203,transport) then
                ...
                if TransportDialog then
                    LOG("*AI DEBUG "..aiBrain.Nickname.." "..UnitPlatoon.BuilderName.." "..repr(transport.PlatoonHandle.BuilderName).." Transport "..repr(transport.EntityId).." UNITS DEAD ends travelwatch after "..watchcount)
                end
