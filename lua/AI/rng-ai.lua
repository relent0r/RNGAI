...
AIBrain = Class(RNGAIBrainClass) {
    ...
    ConfigureDefaultBrainData = function(self)
        ...
        self.EngineerAssistManagerMinAssistPower = 0
        self.EngineerAssistManagerRequests = {}
        ...
        self.EngineerAssistPriorityTables = {
            ['T3AirRush'] = {
                ...
                {cat = categories.MASSEXTRACTION, type = 'Upgrade', debug = 'AirUpgrade mass', bpKey = 'Mass'}, 
        ...
        self.EconomyAllocation = {
            ActualConstructionRatio = 0,
            TargetConstructionRatio = 0.15,
            ConstructionBudgetStatus = 'Healthy'
        }
        ...
        self.BrainIntel.ClosestAllyBase = 0
        ...
        self.BrainIntel.PlayerZoneOwnership = 0
        self.BrainIntel.PlayerZoneOwnership = 0
    ...
    ZoneSetup = function(self)
        ...
        self.Zones.Air = MAP:GetZoneSet('RNGAirResourceSet',0)
        self.ZoneCount = {
            Land = table.getn(self.Zones.Land.zones),
            Naval = table.getn(self.Zones.Naval.zones),
            Air = table.getn(self.Zones.Air.zones)
        }
    ...
    BasesReclaimCheckRNG = function(self)
        ...
        for k, v in self.BuilderManagers do
            if k ~= 'FLOATING' and v.FactoryManager.LocationActive then
                ...
                local searchRings = self.BrainIntel.IMAPConfig.Rings
                if v.FactoryManager.ReclaimRingLimit then
                    --LOG('Ring limit is present, set this for the manager '..tostring(k)..' it is '..tostring(v.FactoryManager.ReclaimRingLimit))
                    searchRings = v.FactoryManager.ReclaimRingLimit
                end
                local cells, count = reclaimGridInstance:FilterAndSortInRadius(gx, gz, searchRings, 25)
                ...
                --
                --RNGLOG('RECLAIM_AUDIT: Base: '..k..' | Radius: '..self.BrainIntel.IMAPConfig.Rings..' | CellsFound: '..count..' | TotalMass: '..totalMassRequired)
            end
        end
    end,
            ...
            if RNGAIGLOBALS.CampaignMapFlag then
                ...
                for index,brain in ArmyBrains do
                    ...
                    elseif IsAlly(myIndex, armyIndex) then
                        ...
                        local playerDistance = VDist3Sq(self.BrainIntel.StartPos, startPos)
                        if armyIndex ~= myIndex and (self.BrainIntel.ClosestAllyBase == 0 or playerDistance < self.BrainIntel.ClosestAllyBase) then
                            self.BrainIntel.ClosestAllyBase = playerDistance
                        end
                    end
                end
                    ...
                    for i = 1, 16 do
                        ...
                        if army and startPos then
                            ...
                            if army.ArmyIndex ~= myArmy.ArmyIndex and (army.Team ~= myArmy.Team or army.Team == 1) then
                                ...
                                enemyStarts[army.ArmyIndex] = {Position = startPos, Index = army.ArmyIndex, Distance = playerDistance, WaterLabels = {}}
                                ...
                                if self.EnemyIntel.ClosestEnemyBase == 0 or playerDistance < self.EnemyIntel.ClosestEnemyBase then
                                    self.EnemyIntel.ClosestEnemyBase = playerDistance
                                end
                            else
                                allyTempStarts[army.ArmyIndex] = {Position = startPos, Index = army.ArmyIndex, WaterLabels = {}}
                                allyStarts['ARMY_' .. i] = startPos
                                if self.BrainIntel.ClosestAllyBase == 0 or playerDistance < self.BrainIntel.ClosestAllyBase then
                                    self.BrainIntel.ClosestAllyBase = playerDistance
                                end
                            end
                        end
                    ...
                    for i = 1, 16 do
                        ...
                        if army and startPos then
                            if army.ArmyIndex == myArmy.ArmyIndex or (army.Team == myArmy.Team and army.Team ~= 1) then
                                ...
                                if army.ArmyIndex ~= myArmy.ArmyIndex and (self.BrainIntel.ClosestAllyBase == 0 or allyDistance < self.BrainIntel.ClosestAllyBase) then
                                    self.BrainIntel.ClosestAllyBase = allyDistance
                                end
            ...
            if self.EcoManager.EcoManagerStatus == 'ACTIVE' then
                ...
                if not powerStateCaution and RNGGETN(unitTypePaused) > 0 then
                    ...
                    for i = RNGGETN(unitTypePaused), 1, -1 do
                        ...
                        while (GetEconomyStoredRatio(self, 'ENERGY') < 0.3 or GetEconomyTrend(self, 'ENERGY') < 5) and recoveryWait < 10 do
                            coroutine.yield(15)
                    ...
                    elseif unitCat.FACTORY then
                        ...
                        elseif unitCat.TECH2 and unitCat.AIR then
                            if v:GetFractionComplete() < 0.7 then
                                ...
                                potentialPowerConsumption = potentialPowerConsumption + (160 * multiplier)
                                continue
                            else
                                v.BuildCompleted = true
                            end
                        ...
                        elseif unitCat.TECH2 and unitCat.LAND then
                            if v:GetFractionComplete() < 0.7 then
                                --RNGLOG('EcoPowerPreemptive : T2 Air Being Built')
                                potentialPowerConsumption = potentialPowerConsumption + (50 * multiplier)
                                continue
                            else
                                v.BuildCompleted = true
                            end
    ...
    RequestEngineerAssistFocus = function(self, source, focusKey, priority, ttl, maxAssist)
        ...
        self.EngineerAssistManagerRequests[source] = {
            FocusKey = focusKey,
            Priority = priority,
            MaxAssistPower = maxAssist,
            Expiry = expiry,
        }
    end,

    GetBrainAssistPriorities = function(self, gameTime)
    ...
    GetTargetBP = function(self, bestFocus, minAssistPower, maxAssistPower)
        ...
        if bestFocus == 'HighValue' and self.EngineerAssistManagerBuildPower <= maxAssistPower then
        ...
        elseif massStorage > 150 and energyStorage > 150 and self.EngineerAssistManagerBuildPower < math.max(minAssistPower, 5) and bestFocus ~= 'HighValue' and not self.EcoManager.CoreMassPush then
        ...
        if energyTrend < 0 and energyPanic then
            if bestFocus ~= 'EnergyRequired' and bestFocus ~= 'SnipeACU' then
    ...
    EngineerAssistManagerBrainRNG = function(self, type)
        -- This is the engineer assist manager brain. It will decide what the AI needs to focus its assisting on.
        -- Note this is global and does not take bases into account.

        local GetPriorityAssistTable = function(focusKey)
            return self.EngineerAssistPriorityTables[focusKey] or self.EngineerAssistPriorityTables['General']
        end
        local FocusCategoryMap = {
            ['AirUpgrade'] = categories.FACTORY * categories.AIR - categories.SUPPORTFACTORY,
            ['LandUpgrade'] = categories.FACTORY * categories.LAND - categories.SUPPORTFACTORY,
            ['EnergyRequired'] = categories.ENERGYPRODUCTION,
            ['T3AirRush'] = categories.FACTORY * categories.AIR * categories.TECH3,
            ['HighValue'] = categories.MOBILE * categories.EXPERIMENTAL + categories.STRUCTURE * categories.EXPERIMENTAL + categories.STRUCTURE * categories.TECH3 * (categories.STRATEGIC + categories.ANTIMISSILE),
            ['General'] = categories.STRUCTURE + categories.EXPERIMENTAL + categories.MOBILE,
            ['SMDLoading'] = categories.STRUCTURE * categories.DEFENSE * categories.ANTIMISSILE * categories.TECH3,
        }
        ...
        while true do
            ...
            local bestForceMaxAssist = false
            local currentTime = GetGameTimeSeconds()
            self:GetBrainAssistPriorities(currentTime)

            if self.EngineerAssistManagerRequests then
                for source, data in self.EngineerAssistManagerRequests do
                    if currentTime < data.Expiry then
                        if data.Priority > bestPriority then
                            bestPriority = data.Priority
                            bestFocus = data.FocusKey
                        end
                    else
                        -- Prune expired entry
                        self.EngineerAssistManagerRequests[source] = nil
                    end
                end
            end

            self.EngineerAssistManagerPriorityTable = GetPriorityAssistTable(bestFocus)
            self.EngineerAssistManagerFocusCategoryLookup = bestFocus
            self.EngineerAssistManagerFocusCategory = FocusCategoryMap[bestFocus] or FocusCategoryMap['General']
            self.EngineerAssistManagerMinAssistPower = targetBp
            targetBp = self:GetTargetBP(bestFocus, minAssistPower, maxAssistPower)
    ...
    AdjustEconomicAllocation = function (self)
        ...
        while self.Status ~= 'Defeat' do
            ...
            local cfg = self.EcoConfig or {
                ...
                projectionEnemyUnitsLand = 0.5,
                
                -- Threat Differential Evaluation Weights
                landGapWeight = 0.90,
                navalGapWeight = 0.90,
                
                -- Local Expansion Area Threat Divisors
                landThreatDivisor = 60.0,
                airThreatDivisor = 45.0,
                navalThreatDivisor = 60.0,
                
                -- Structural Expansion Panic Controls
                expansionDampenerMin = 0.5,
                expansionDampenerFactor = 0.5,
                phaseUpgradeThreshold = 1,
                phaseUpgradeMassTrendMin = -1.0,
                ...
                -- System Pressure Models
                gravityPressureDivisor = 500.0,
                localClaimsMaxClamp = 0.50,      -- Maximum reduction of Expansion pressure by local claims
                highPressureThreshold = 0.25,
                minProdBudget = 0.20,
                maxProdBudget = 0.85,
                
                -- Construction Budget Status Allocation
                targetConstRatioDefault = 0.15,
                targetConstRatioHighPressure = 0.10,
                pressureHighThreshold = 0.4,
                constRatioOverageMultiplier = 1.5,
                
                -- Strategic Shifts & Dominance Caps
                landShiftFactor = 0.6,
                ...
                assistPressureWeight = 0.15,
                weightSafetyBuffer = 0.01,
                
                -- Allocation Clamping & Redistribution
                excessEcoFactor = 0.4,
                excessAssistFactor = 0.6,
                trendFactorMin = 0.5,
                trendFactorMax = 1.2,
                scaleFloor = 0.1,
            ...
            local economicDebugEnabled = cfg.debugEnabled
            local threatFactorThreshold = cfg.threatFactorThreshold
            coroutine.yield(30)
            ...
            local im = self.IntelManager
            local myThreat = brainIntel.SelfThreat or {}
            ...
            local engSpendData = (self.cmanager and self.cmanager.categoryspend and self.cmanager.categoryspend.eng) or {}
            local totalEngSpend = (engSpendData.T1 or 0) + (engSpendData.T2 or 0) + (engSpendData.T3 or 0) + (engSpendData.com or 0)
            local actualConstRatio = safeDiv(math.max(0, totalEngSpend - assistConsumption), math.max(1, totalIncome), 0)
            ...
            local ecoFloor = cfg.ecoFloorFactor * highestPhase
            local assistFloor = cfg.assistFloorFactor * highestPhase
            ...
            local smFactories = sm and sm.Factories or {}

            local function hasFactory(domain)
                local d = smFactories[domain]
                if not d then return false end
                return ((d[1] and d[1].Total or 0) > 0)
                    or ((d[2] and d[2].Total or 0) > 0)
                    or ((d[3] and d[3].Total or 0) > 0)
            end
            ...
            local landProjection = cfg.projectionDefaultLand
            local navalProjection = cfg.projectionDefaultNaval
            ...
            if currentEnemy and self.CanPathToEnemyRNG then
                ...
                if mainPath and mainPath ~= 'LAND' and labelCount > 0 then
                    if (self.MapWaterRatio or 0) > cfg.mapWaterRatioThreshold then
                        isNavalMap = true
                    end
                end
                if mainPath ~= 'LAND' then
                    landProjection = cfg.projectionStalledLand
                    ...
                    if myLabel and im.LabelIMAPThreat and im.LabelIMAPThreat['Land'] then
                        ...
                        if labelThreat then
                            if (labelThreat.enemystructurethreat or 0) > 0 then
                                landProjection = cfg.projectionEnemyStructureLand
                            elseif (labelThreat.enemylandthreat or 0) > 0 then
                                landProjection = cfg.projectionEnemyUnitsLand
                            end
                        end
                    end
                end
            ...
            local currentBias =
                (brainIntel.PlayerRole.AirPlayer and cfg.biases.Air)
                or (brainIntel.PlayerRole.SpamPlayer and cfg.biases.Land)
                or (isNavalMap and cfg.biases.Naval)
                or ((self.EnemyIntel and self.EnemyIntel.ChokeFlag) and cfg.biases.ChokePoint)
                or cfg.biases.Default

            local navalBiasMultiplier = (im and im.NavalFocusSafe) and cfg.navalFocusMultiplier or 1.0
            ...
            local projectedLandRisk  = clamp(safeDiv(landGap * landProjection * cfg.landGapWeight, math.max(1, enemyThreat.Land or 0), 0), 0, 1)
            local projectedNavalRisk = clamp(safeDiv(navalGap * navalProjection * cfg.navalGapWeight, math.max(1, enemyThreat.Naval or 0), 0), 0, 1)
            if economicDebugEnabled then
                ...
                LOG(string.format("RNGLOG_FFA_AUDIT | LandGap: %.2f | RawEnemyLand: %.2f | MyLand: %.2f | W_Risk: %.2f", landGap, (enemyThreat.Land or 0), (myThreat.LandNow or 0), projectedLandRisk))
            ...
            for layerKey, zoneList in productionZonesByLayer do
                for _, pZone in zoneList do
                    ...
                    if fmgr and fmgr.LocationActive then
                        ...
                        local lLand = clamp((fmgr.ZoneThreatAssignment or 0) / cfg.landThreatDivisor, 0, 1)
                        local monitor = self.BasePerimeterMonitor[pZone.BuilderManager.LocationType]
                        local lAir  = monitor and clamp((monitor.AirThreat or 0) / cfg.airThreatDivisor, 0, 1) or 0
                        local lNaval = (layerKey == 'Naval') and clamp((fmgr.ZoneThreatAssignment or 0) / cfg.navalThreatDivisor, 0, 1) or 0
                        ...
                        if managerLocalPressure > 0 and totalIncome > 0 then
                            ...
                            local dampener = isMain and 1.0 or (cfg.expansionDampenerMin + (projectedLandRisk * cfg.expansionDampenerFactor))
            ...
            if brainIntel.PlayerStrategy and brainIntel.PlayerStrategy.T3AirRush then
                engineerAssistRatio = engineerAssistRatio + cfg.t3AirRushAssistBonus
            end

            local minAllocation = cfg.minAllocation
            local maxShiftLandRatio, maxShiftAirRatio, maxShiftNavalRatio = cfg.maxShiftLandDefault, cfg.maxShiftAirDefault, cfg.maxShiftNavalDefault

            if brainIntel.PlayerRole.SpamPlayer then
                maxShiftLandRatio = cfg.maxShiftLandSpam
            ...
            local gravityPressure = clamp(brainIntel.GlobalEconomicGravity / cfg.gravityPressureDivisor, 0, 1)
            ...
            if combinedPressure > cfg.pressureHighThreshold then targetConstRatio = cfg.targetConstRatioHighPressure end
            self.EconomyAllocation.TargetConstructionRatio = targetConstRatio
            
            if actualConstRatio > targetConstRatio * cfg.constRatioOverageMultiplier then self.EconomyAllocation.ConstructionBudgetStatus = 'Blocked'
            elseif actualConstRatio > targetConstRatio then self.EconomyAllocation.ConstructionBudgetStatus = 'Limited'
            else self.EconomyAllocation.ConstructionBudgetStatus = 'Healthy' end

            local isHighPressure = math.max(projectedLandRisk, projectedNavalRisk, combinedPressure) > cfg.highPressureThreshold
            ...
            local defaultLand  = (self.DefaultProductionRatios and self.DefaultProductionRatios.Land)  or 0
            local defaultAir   = (self.DefaultProductionRatios and self.DefaultProductionRatios.Air)   or 0
            local defaultNaval = (self.DefaultProductionRatios and self.DefaultProductionRatios.Naval) or 0

            local landTotalThreat  = math.max(1, (enemyThreat.Land or 0)    + (myThreat.LandNow or 0)    + (myThreat.AllyLandThreat or 0))
            local airTotalThreat   = math.max(1, (enemyThreat.AntiAir or 0) + (myThreat.AntiAirNow or 0) + (myThreat.AllyAntiAirThreat or 0))
            local navalTotalThreat = math.max(1, (enemyThreat.Naval or 0)   + (myThreat.NavalNow or 0)   + (myThreat.AllyNavalThreat or 0))

            local desiredLand, desiredAir, desiredNaval = 0, 0, 0

            if hasLandProduction then
                local normalized = defaultLand * productionAllocation
                local shift = (landGap > 0) and (safeDiv(landGap, landTotalThreat, 0) * cfg.landShiftFactor) or 0
                desiredLand = math.max(minAllocation, math.min(normalized + (shift * maxShiftLandRatio * currentBias.Land), maxShiftLandRatio))

                if ((myThreat.LandNow or 0) + (myThreat.AllyLandThreat or 0)) > ((enemyThreat.Land or 0) * threatFactorThreshold) and projectedLandRisk < cfg.dominanceRiskThreshold then
            ...
            if hasAirProduction then
                local normalized = defaultAir * productionAllocation
                local shift = (airGap > 0) and (airGap / airTotalThreat) or 0
            ...
            if hasNavalProduction then
                local normalized = defaultNaval * productionAllocation
                ...
                if ((myThreat.NavalNow or 0) + (myThreat.AllyNavalThreat or 0)) > ((enemyThreat.Naval or 0) * threatFactorThreshold) and projectedNavalRisk < cfg.dominanceRiskThreshold then
                    excessAllocation = excessAllocation + math.max(0, desiredNaval - minAllocation)
                    desiredNaval = minAllocation
                end
            end

            local newLand, newAir, newNaval = desiredLand, desiredAir, desiredNaval
            productionAllocation = math.max(minProd, productionAllocation - excessAllocation)
            local totalUnitNeeds = newLand + newAir + newNaval

            if totalUnitNeeds > 0 then
                local normFactor = productionAllocation / totalUnitNeeds
                newLand  = newLand  * normFactor
                newAir   = newAir   * normFactor
                newNaval = newNaval * normFactor
            end
            self.ProductionIntent = {
                Land = newLand,
                Air = newAir,
                Naval = newNaval
            }
            ...
            ----------------------------------------------------------------------
            if totalIncome > 0 and (not isHighPressure or highestPhase > 2) and self.EcoManager then
                local caps = {
                    Land  = safeDiv(self.EcoManager.ApproxLandFactoryMassConsumption or 0, totalIncome, 0),
                    Air   = safeDiv(self.EcoManager.ApproxAirFactoryMassConsumption or 0, totalIncome, 0),
                    Naval = safeDiv(self.EcoManager.ApproxNavalFactoryMassConsumption or 0, totalIncome, 0),
                }

                if newLand > caps.Land then
                    excessAllocation = excessAllocation + (newLand - caps.Land)
                    newLand = caps.Land
                end
                if newAir > caps.Air then
                    excessAllocation = excessAllocation + (newAir - caps.Air)
                    newAir = caps.Air
                end
                if newNaval > caps.Naval then
                    excessAllocation = excessAllocation + (newNaval - caps.Naval)
                    newNaval = caps.Naval
                end
            ...
            if economicDebugEnabled then
                local currentCap = (self.EcoManager and self.EcoManager.ApproxLandFactoryMassConsumption or 0) / math.max(1, totalIncome)
                local phantomLand = math.max(0, (newLand or 0) - currentCap)
                LOG(string.format("RNGLOG_SQUEEZE_AUDIT | Pressure: %.2f | LandIntent: %.2f | LandCap: %.2f | PhantomWaste: %.2f | AssistRatio: %.2f", combinedPressure, (newLand or 0), currentCap, phantomLand, engineerAssistRatio))
            end
            ...
            if excessAllocation > 0 then
                economyUpgradeSpend = clamp(economyUpgradeSpend + (excessAllocation * cfg.excessEcoFactor), ecoFloor, maxEcoSpend)
            ...
            ----------------------------------------------------------------------
            self.ProductionRatios.Land = newLand
            self.ProductionRatios.Air = newAir
            self.ProductionRatios.Naval = newNaval
            ...
            if economicDebugEnabled then
                ...
                LOG(string.format("RNGLOG_DEBUG_ALLOC_FINAL | Income: %.2f | TotalEngSpend: %.2f | ActualConstRatio: %.2f | Leftover: %.2f | EcoSpend: %.2f | AssistRatio: %.2f | ExcessAlloc: %.2f",
                    totalIncome, totalEngSpend, actualConstRatio, leftover, economyUpgradeSpend, engineerAssistRatio, excessAllocation))
                LOG(string.format("RNGLOG_DEBUG_ALLOC_FINAL_RAW | TotalUnitAlloc: %.2f | LeftoverAfterEco: %.2f | BudgetStatus: %s | TrendFactor: %.2f",
                    totalUnitAllocation, leftoverAfterEco, self.EconomyAllocation.ConstructionBudgetStatus, trendFactor))
                LOG(string.format("OBTP_SQUEEZE | ProdAlloc: %.2f | EcoSpend: %.2f | LandWanted: %.2f",productionAllocation, economyUpgradeSpend, newLand))

                -- TUNING HEARTBEAT: The absolute summary for loss analysis.
                -- Correlation: [Time] Income vs BudgetStatus | MyTech vs EnemyTech | ArmyGaps | Allocation%
